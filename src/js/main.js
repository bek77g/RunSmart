...
//------------
  $('.catalog__caption').each(function(i) {
    ...
    if (storage) {
      $(this).find('li').removeClass('catalog__item-active').eq(storage).addClass('catalog__item-active')
      .closest('div.catalog__tabs').find('div.catalog__content').removeClass('catalog__content-active').eq(storage).addClass('catalog__content-active');
    }
  });


  $('.catalog__caption').on('click', 'li:not(.catalog__item-active)', function() {
    $(this)
    .addClass('catalog__item-active').siblings().removeClass('catalog__item-active')
    .closest('div.catalog__tabs').find('div.catalog__content').removeClass('catalog__content-active').eq($(this).index()).addClass('catalog__content-active');
...
//переходы
  ...
  var map;
    ...
    DG.then(function () {
        ...
        var marker = DG.marker([42.843731, 74.623135],{icon: myIcon}).addTo(map).bindPopup('Вы кликнули по мне!');
